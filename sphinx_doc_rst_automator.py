...
class Main:
    ...
    def populate_files(self):
        docs_path = os.path.dirname(os.path.dirname(self.chosen_source_folder)) + "/docs"
        os.mkdir(docs_path)
        os.mkdir(docs_path + "/source")
        ...
        if self.chosen_source_folder == "":
            ...
            for line in self.files_field.get("1.0", tk.END).split("\n"):
                if line != "":
                    if line[-3:] == ".py":
                        ...
                        if file_name != "":
                            # Write to file
                            with open(f"{docs_path}/source/{file_name}", "w+") as file:
            ...
            with open("index_contents.txt", "r") as index_source_file:
                with open(f"{docs_path}/source/index.rst", "w+") as file:
                    file.write(index_source_file.read())

            with open("getting_started_contents.txt", "r+") as getting_started_source_file:
                with open(f"{docs_path}/source/getting_started.rst", "w+") as file:
                    file.write(getting_started_source_file.read())

            with open(f"{docs_path}/generate.bat", "w+") as file:
                directory = os.path.dirname(self.chosen_source_folder)
                file.write(f"sphinx-autobuild source build/html --watch {directory}\npause")

            with open(f"{docs_path}/source/index.rst", 'r') as index_file:
            ...
            with open(f"{docs_path}/source/conf.py", 'w+') as file:
                with open("conf_contents.txt","r") as conf_source_file:
                    file.write(conf_source_file.read())

            os.replace("temp_file.rst", f"{docs_path}/source/index.rst")
